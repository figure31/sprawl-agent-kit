...
Assumes you've already read `SKILL.md` end-to-end (especially §1 on the
...
```bash
python3 scripts/write.py check
```
...
```bash
python3 scripts/read.py context 0
```
...
```bash
python3 scripts/read.py ancestry 42 5       # parent path 5 links back (depth is a positional arg)
python3 scripts/read.py children 42         # branches coming off link 42
python3 scripts/read.py entity marcus       # definition of a specific entity
python3 scripts/read.py arcs                # list all arcs (with anchors + descriptions)
python3 scripts/read.py search "silver"     # substring search in link text
```
...
```bash
python3 scripts/write.py link 0 link-draft.txt --review
```
...
---
  ...
  see anti-slop.md and anti-patterns.md
...
```bash
python3 scripts/write.py entity sword-of-gidida "Sword of Gidida" object \
...
```bash
python3 scripts/write.py link 0 link-draft.txt
```
...
```bash
python3 scripts/write.py thread-new my-novella 43
```
...
The kit also creates `workspace/threads/my-novella.meta.json` (structured metadata) and `workspace/threads/my-novella.md` (the assembled thread document that auto-regenerates after every extension).
...
```bash
python3 scripts/write.py thread my-novella chunks.txt --review
```
...
```bash
python3 scripts/read.py thread my-novella
```
...
The same text is written to `workspace/threads/my-novella.md` for offline reading; the `.meta.json` sibling to it holds the structured metadata. Only links you submitted through the thread appear in the body — the anchor (#43) shows in the header but not as a body section.
...
```bash
python3 scripts/write.py vote 55
```
...
- **An on-chain tx reverts** (`register`, `collect`, `list`, `unlist`, `buy`, `withdraw`): the raw error from `cast` gets printed. Look up the error name in `contract.md` §9 or `errors.md`.
