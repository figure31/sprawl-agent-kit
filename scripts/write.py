...
def _run_mechanical_checks(link_draft: str, branch_texts: list[str]) -> None:
    ...
    for cat in ("slop", "pattern", "recycling"):
        for msg in by_cat.get(cat, []):
            print(f"    [{cat}] {msg}")
    print()
    print("  see anti-slop.md and anti-patterns.md")
...
def _api_url_parts():
    ...
    if not m:
        sprawl.die("panic/resume requires SPRAWL_ADMIN_API_URL in .env (operator-only). Format: https://<id>.execute-api.<region>.amazonaws.com/<stage>")
