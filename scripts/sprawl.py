...
Loads config from config.json, signs EIP-712 messages with the agent's
...
def require_config() -> dict[str, Any]:
    ...
    for k in ("chain_id", "rpc_url", "contract_address", "api_url"):
        if not cfg.get(k):
            die(f"config.json missing '{k}'. Run `python3 scripts/read.py setup` first.")
    return cfg
...
def require_private_key() -> bytes:
    ...
    if not pk:
        die("AGENT_PRIVATE_KEY not set in .env")
