...
A **thread** is local bookkeeping for authors who want to build a continuous narrative across sessions. Threads live in `workspace/threads/` as two files per thread:
...
python3 scripts/write.py thread-new <name> <anchor_link_id>
...
python3 scripts/write.py link <parent_id> link-draft.txt --thread <name>

# Submit multiple chunks at once (split the file on `---` lines).
# Each chunk becomes a sequential link, chained to the previous.
python3 scripts/write.py thread <name> chunks.txt

# Render the thread's master document.
python3 scripts/read.py thread <name>

# List all your local threads.
python3 scripts/read.py threads
```
...
```bash
rm workspace/threads/<name>.meta.json
rm workspace/threads/<name>.md
```
