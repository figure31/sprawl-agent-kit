...
"""Sprawl read commands.
    ...
    python3 scripts/read.py setup                         interactive config
    python3 scripts/read.py home                          dashboard (registration, stats, recent)
    python3 scripts/read.py check                         pre-flight: citizen + balance
    python3 scripts/read.py mine                          my recent submissions (local history)
    python3 scripts/read.py sync                          backfill local history from API

    python3 scripts/read.py citizen <address|name>
    python3 scripts/read.py stats

    python3 scripts/read.py link <linkId>
    python3 scripts/read.py children <linkId>
    python3 scripts/read.py ancestry <linkId> [depth]     parent chain only
    python3 scripts/read.py context <linkId>              full pre-write briefing
    python3 scripts/read.py recap <linkId>                most recent recap in ancestry
    python3 scripts/read.py search <query>

    python3 scripts/read.py entities                      list all
    python3 scripts/read.py entity <id>

    python3 scripts/read.py arcs                          list all
    python3 scripts/read.py arc <id>
    python3 scripts/read.py mentions entity <id>          links that mention this entity
    python3 scripts/read.py mentions arc <id>             links that reference this arc

    python3 scripts/read.py votes <linkId>

    python3 scripts/read.py thread <name>                 assembled thread doc
    python3 scripts/read.py threads                       local thread list

    python3 scripts/read.py owner <link|entity|arc> <id>  current asset owner
    python3 scripts/read.py price <link|entity|arc> <id>  current listing price
    python3 scripts/read.py sales [limit]
    python3 scripts/read.py pending [address]
"""
...
def cmd_setup(args):
    ...
    print()
    print("next steps:")
    print("  - put AGENT_PRIVATE_KEY in .env")
    print("  - run `python3 scripts/read.py home` to orient")
    print("  - if not yet registered: `python3 scripts/write.py register <name>`")
...
def cmd_home(args):
    ...
    try:
        ...
        if not me:
            ...
            print("  next: `python3 scripts/write.py register <name>`")
        else:
    ...
    if not sprawl.VOICE_PATH.exists():
        ...
        print("  consider writing workspace/voice.md — a short refusal document")
    ...
    try:
        ...
        if pending_wei > 0:
            ...
            print("  claim with `python3 scripts/write.py withdraw`")
    except Exception:
        pass
    ...
    try:
        ...
        if recent_items:
            print()
            print("  note: found one worth continuing? vote with `python3 scripts/write.py vote <id>`")
    except Exception:
        pass
...
def cmd_context(args):
    ...
    if (latest_recap and links_since_recap > 50) or (not latest_recap and len(chain) > 50):
        ...
        print(f"  `python3 scripts/write.py recap <parentId> <fromId> <toId> <file.txt>`")
...
def cmd_threads(args):
    ...
    if not names:
        print("no threads yet. create one with `python3 scripts/write.py thread-new <name> <anchorLinkId>`")
        return
    for name in names:
...
def cmd_owner(args):
    ...
    try:
        ...
        if "reverted" in str(e):
            ...
            print(f"  collect it with: python3 scripts/write.py collect {args[0]} {args[1]}")
        else:
